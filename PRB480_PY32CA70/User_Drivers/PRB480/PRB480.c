...
#define PRB480_TSLOT_US             54      /* 图 12：单总线时隙，手册 125Kbits/s 典型 30us */
...
#define PRB480_TLOW_NOP_COUNT    4U      /* 读时序拉低总线时间 */
...
#define TMX_DELAY_US_NUM    1U      /* CA70 初始使用标准 SysTick 时基 */
#define TMX_DELAY_US_DEN    1U      /* 上板后再根据示波器实测校准 */
...
void PRB480_BoardInterfaceConfig(void)
{
    ...
    LL_GPIO_SetPinMode(PRB480_RESP_GPIO_PORT, PRB480_RespPin, LL_GPIO_MODE_OUTPUT);
    LL_GPIO_SetPinOutputType(PRB480_RESP_GPIO_PORT, PRB480_RespPin, LL_GPIO_OUTPUT_PUSHPULL);
    LL_GPIO_SetPinPull(PRB480_RESP_GPIO_PORT, PRB480_RespPin, LL_GPIO_PULL_NO);
    LL_GPIO_SetPinSpeed(PRB480_RESP_GPIO_PORT, PRB480_RespPin, LL_GPIO_SPEED_FREQ_VERY_HIGH);
    LL_GPIO_SetPinMode(PRB480_POWER_GPIO_PORT, PRB480_PowerPin, LL_GPIO_MODE_OUTPUT);
    LL_GPIO_SetPinOutputType(PRB480_POWER_GPIO_PORT, PRB480_PowerPin, LL_GPIO_OUTPUT_PUSHPULL);
    LL_GPIO_SetPinPull(PRB480_POWER_GPIO_PORT, PRB480_PowerPin, LL_GPIO_PULL_NO);
    LL_GPIO_SetPinSpeed(PRB480_POWER_GPIO_PORT, PRB480_PowerPin, LL_GPIO_SPEED_FREQ_VERY_HIGH);
...
/**
 * @brief 将 PRB480 的 DQ 引脚配置为 ADC 模拟输入模式
 *
 * @note
 * 1. 调用本函数前，必须已经使能 DQ 引脚对应的 GPIO 时钟。
 * 2. 模拟输入模式下应关闭内部上拉和下拉，避免影响 ADC 采样电压。
 * 3. GPIO 输出速度对模拟输入没有实际作用，此处设置为低速。
 */
static void PRB480_IO_ANALOG(void)
{
    /*
     * 将 PRB480 DQ 引脚配置为模拟模式。
     * 进入模拟模式后，GPIO 数字输入输出电路关闭，
     * 引脚电压可以送入 ADC 进行采样。
     */
    LL_GPIO_SetPinMode(PRB480_DqPort,
                       PRB480_DqPin,
                       LL_GPIO_MODE_ANALOG);

    /*
     * 关闭 DQ 引脚的内部上拉和下拉电阻。
     * 内部上下拉可能改变总线电压，从而影响 ADC 采样值
     * 以及 PRB480 的高低电平判断。
     */
    LL_GPIO_SetPinPull(PRB480_DqPort,
                       PRB480_DqPin,
                       LL_GPIO_PULL_NO);
     ...
     * 将 GPIO 输出速度设置为低速。
     * 模拟输入模式不使用 GPIO 输出驱动，
     * 因此速度配置不会影响 ADC 的转换速度。
     */
    LL_GPIO_SetPinSpeed(PRB480_DqPort,
                        PRB480_DqPin,
...
static void PRB480_ADC_Config(void)
{
    ...
    LL_ADC_SetSamplingTimeCommonChannels(ADC1, LL_ADC_SAMPLINGTIME_6CYCLES_5);
    LL_ADC_REG_SetSequencerLength(ADC1, LL_ADC_REG_SEQ_SCAN_DISABLE);
    LL_ADC_REG_SetSequencerRanks(ADC1, LL_ADC_REG_RANK_1, PRB480_ADC_CHANNEL);
    ...
    if (LL_ADC_IsEnabled(ADC1) == 0U)
    {
        ...
        timeout = 10000U;
        while ((LL_ADC_IsCalibrationOnGoing(ADC1) != 0U) && (timeout > 0U)) timeout--;
        if (timeout == 0U)
        {
            printf("PRB480 ADC calibration timeout\r\n");
            return;
        }
        LL_ADC_Enable(ADC1);
    }
